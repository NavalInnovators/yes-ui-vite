...
export default function Mycart() {
    ...
    const [isLoadingSuggestedCourses, setIsLoadingSuggestedCourses] =
        useState(false);
            ...
            const options = {
                ...
                handler: async function (response) {
                    ...
                    try {
                        ...
                        const enrollingToast = toast.loading(
                            "Enrolling in courses...",
                        );

                        try {
                            const enrollmentPromises = purchasedCourses.map(
                                async (cartItem) => {
                                    try {
                                        ...
                                        const actualCourse = allCourses.find(
                                            (course) =>
                                        ...
                                        console.error(
                                            `Enrollment failed for ${cartItem.name}:`,
                                            enrollError,
                                        );
                            ...
                            if (successfulEnrollments.length > 0) {
                                ...
                                const existingMyCourses = JSON.parse(
                                    localStorage.getItem("myCourses") || "[]",
                                );
                                const updatedMyCourses = [...existingMyCourses];

                                successfulEnrollments.forEach(
                                    (enrolledCourse) => {
                                        const alreadyExists =
                                            updatedMyCourses.some(
                                                (course) =>
                                                    course.id ===
                                                    enrolledCourse.id,
                                            );
                                        if (!alreadyExists) {
                                            updatedMyCourses.push(
                                                enrolledCourse,
                                            );
                                        }
                                    },
                                ...
                                localStorage.setItem(
                                    "myCourses",
                                    JSON.stringify(updatedMyCourses),
                                );

                                const { getBookDetails } = await import(
                                    "./sharedQuery"
                                );
                                localStorage.setItem(
                                    "bookDetails",
                                    JSON.stringify(
                                        getBookDetails(updatedMyCourses),
                                    ),
                                );
                            }
                        ...
                        try {
                            ...
                            toast.dismiss(loadingToast);
                            toast.success(
                                "Payment successful! Courses activated.",
                            );
                            ...
                            toast.dismiss(loadingToast);
                            toast.success(
                                "Payment successful! Please check your courses.",
                            );
    ...
    const handleApplyCoupon = async (coupon) => {
        try {
            ...
            const appliedCouponData = {
                ...
                minCourseSelection:
                    response.minCourseSelection ||
                    coupon?.minCourseSelection ||
                    0,
                activationAmount:
    ...
    const bestCoupon = useMemo(() => {
        ...
        availableCoupons.forEach((coupon) => {
            ...
            if (
                coupon.minCourseSelection &&
                cart.length < coupon.minCourseSelection
            )
                return;
            if (coupon.activationAmount && subtotal < coupon.activationAmount)
                return;
    ...
    return (
        <div className="w-full min-h-screen pt-20 md:pt-28">
            ...
            {isLoadingCart || isAddingToCart ? (
                ...
                // Empty cart state
                <div className="px-4 py-6 md:p-10 flex flex-col justify-center items-center min-h-[60vh]">
                    <NoData
                        title="Your Cart is Empty"
                        message="Take the next step in your learning journey. Browse and choose courses made for you."
                    />
                    <button
                                                ...
                                                const fetchSuggestedCourses =
                                                    async () => {
                                                        try {
                                                            ...
                                                            setIsLoadingSuggestedCourses(
                                                                true,
                                                            );
                                                            setSuggestedCoursesError(
                                                                null,
                                                            );
                                                            ...
                                                            setSuggestedCoursesError(
                                                                "Failed to load suggested courses",
                                                            );
                                                        } finally {
                                                            setIsLoadingSuggestedCourses(
                                                                false,
