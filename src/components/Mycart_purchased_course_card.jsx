...
export default function Mycart_purchased_course_card({ 
  ...
  return (
    <div className={`w-full max-w-md relative flex items-center justify-between ${bgClass} ${textClass} rounded-2xl shadow-md p-4 ${showSparkle ? 'animate-pulse' : ''}`}>
      {/* Remove button */}
      <button 
        onClick={onRemove}
        className={`absolute top-2 right-2 ${isPro ? 'text-white hover:text-red-300' : 'text-gray-600 hover:text-red-600'} transition-colors`}
      >
        ✕
      </button>
