    ...
    <div className="w-full max-w-sm bg-white rounded-3xl shadow-xl p-8 flex flex-col border border-gray-100" style={{minHeight: 'fit-content', maxHeight: '90vh'}}>
        ...
        {appliedCoupon ? (
          <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
            <div className="flex justify-between items-center">
              <div>
                <div className="font-semibold text-green-800">{appliedCoupon.name}</div>
      ...
      <div className="space-y-3 text-base text-gray-700 mt-6">
        <div className="flex justify-between">
          <span>Subtotal</span>
          <span className="text-gray-900 font-semibold">₹{pricing.subtotal}</span>
        </div>
        {pricing.discount > 0 && (
          <div className="flex justify-between">
            <span>Discount</span>
        ...
        <>
          {cart.length <= 4 ? (
            <div className="mt-4 bg-gradient-to-r from-green-50 to-green-100 border border-green-200 rounded-xl py-3 px-4 text-center text-sm text-green-800 shadow-sm">
              <span className="font-semibold">Select 5+ courses to unlock 25% discount!</span>
            </div>
          ) : (
            <div className="mt-4 bg-gradient-to-r from-purple-50 to-purple-100 border border-purple-200 rounded-xl py-3 px-4 text-center text-sm text-purple-800 shadow-sm">
              <span className="font-semibold">Upgrade to Pro, get 30% discount!</span>
            </div>
          )}
          ...
          {pricing.hasBasic && (
            <div className="mt-4 bg-gray-50 border border-gray-200 rounded-xl p-4">
              <div className="flex justify-between items-center">
                <div>
                  <div className="font-semibold text-gray-800">Bundle Savings</div>
                  <div className="text-sm text-gray-600">Upgrade all to Pro & Save 30%</div>
                </div>
                <button 
      ...
      <div className="text-xl font-bold text-gray-900 text-center mb-4">
        ...
        <span className="ml-2 font-extrabold text-purple-700">₹{pricing.total}</span>
      </div>
